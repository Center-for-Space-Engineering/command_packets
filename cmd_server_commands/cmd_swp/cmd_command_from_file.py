...
class cmd_command_from_file(commandParent):
    ...
    def __init__(self, CMD, coms):
        ...
        self.__args ={
            ...
            "enter_calibration_mode" : self.enter_calibration_mode
        }
    ...
    def enter_calibration_mode(self, _):
        '''
        ...
        self.__coms.print_message(dto, 2)
        return f"<p>ran command enter_calibration_mode</p><p>{formatted_bytes}</p> " + return_val
    ...
    def get_args_server(self):
        ...
        for key in self.__args:
            ...
            elif key == 'enter_calibration_mode':
                message.append({ 
                'Name' : key,
                'Path' : f'/{self.__commandName}/{key}/',
                'Description' : 'Commands SWP2 to Enter Calibration Mode (only possible from IDLE mode)'    
                })
        return message
