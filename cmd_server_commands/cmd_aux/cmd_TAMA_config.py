...
class cmd_TAMA_config(commandParent):
    '''
    ...
    def __init__(self, CMD, coms):
        ...
        self.__commandName = 'command_TAMA_config'
        self.__args ={
            "TAMA_config" : self.TAMA_config
        }
    ...
    def run_args(self, args):
        ...
        return message
    def TAMA_config(self, args):
        '''
            creates a byte array for a TAMA config packet.
        '''
        ...
        packet_length = 9 # number of bytes after header
        ...
        USU_DEL_bytes = USU_DEL.to_bytes(4, 'big')
        
        USU_CYC_bytes = USU_CYC.to_bytes(4, 'big')
        
        bytes_for_crc = header + USU_DEL_bytes + USU_CYC_bytes
        ...
        dto = print_message_dto("Ran TAMA_config")
        self.__coms.print_message(dto, 2)
        return f"<p>ran command TAMA_config with args {str(args)}</p><p>{formatted_bytes}</p><p>{return_val}</p>"
    def get_args(self):
        '''
    ...
    def get_args_server(self):
        ...
        for key in self.__args:
            if key == 'TAMA_config':
                message.append({ 
                'Name' : key,
                'Path' : f'/{self.__commandName}/{key}/-USU_DEL-/-USU_CYC-',
                'Description' : 'Configures Three-Axis mag parameters'    
                })
        return message